...
///
/// Captures the DRM plane via kmsgrab + system audio via PipeWire's
/// HDMI sink monitor port (auto-generated by every PipeWire sink — it
/// carries everything mixed to that sink, regardless of source). Emits
/// a single MPEG-TS over SRT. No local recording — use the separate
/// `CaptureService.startRecording` flow if you want an MP4.
///
/// The PULSE_SERVER env var passed via sudo points at the hearth user's
/// PipeWire-pulse compatibility socket. The socket is `srw-rw-rw-`, so
/// the root ffmpeg process can connect cleanly.
Future<StreamingProcess> ffmpegStartStream(String host, int port) async {
  final proc = await Process.start('sudo', [
    ...
    '-i',
    '-',
    // Audio input: PipeWire's HDMI sink monitor via pulse compatibility.
    // pulse handles its own buffering well, so no -thread_queue_size needed.
    '-f',
    'pulse',
    '-i',
    'alsa_output.platform-107c701400.hdmi.hdmi-stereo.monitor',
